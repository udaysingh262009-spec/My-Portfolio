...
export default function LearningJourneySection() {
  ...
  return (
    <section className="py-20 px-6 sm:px-10 lg:px-14 max-w-[1600px] mx-auto z-10 space-y-14">
      {/* Section Header */}
      <div className="flex flex-col items-center text-center space-y-3">
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full glass-pill border border-emerald-500/30 text-xs font-semibold text-emerald-400 uppercase tracking-wider">
          <FaRocket size={12} />
      ...
      <div className="relative border-l-2 border-emerald-500/30 ml-4 sm:ml-8 lg:ml-12 pl-6 sm:pl-10 space-y-10 text-left">
        {steps.map((step, idx) => (
          <motion.div
            key={idx}
          ...
          >
            ...
            {/* Glass Content Card */}
            <div className="glass-panel p-6 sm:p-7 rounded-3xl border border-emerald-500/20 shadow-xl space-y-3 hover:border-emerald-400/50 transition-all duration-300">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <span className="px-3 py-1 rounded-full glass-pill text-xs font-mono font-bold text-emerald-400 border border-emerald-500/30">
                  {step.year}
                </span>
                <span className="text-xs text-gray-400 font-mono font-medium">{step.subtitle}</span>
              </div>
