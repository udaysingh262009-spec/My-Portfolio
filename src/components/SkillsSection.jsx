...
export default function SkillsSection() {
  ...
  return (
    <section id="skills" className="py-16 sm:py-20 px-4 sm:px-8 lg:px-12 max-w-[1600px] mx-auto z-10">
      {/* Section Header */}
      ...
      <div className="glass-panel p-6 sm:p-10 rounded-3xl border border-emerald-500/20 shadow-2xl space-y-6">
        ...
        {/* Skills Cards Grid */}
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-3 gap-4 text-left max-w-5xl mx-auto">
          {filteredSkills.map((sk) => (
            <div
              key={sk.name}
              className="p-4 rounded-2xl glass-panel border border-white/5 hover:border-emerald-400/40 transition-all duration-300 group hover:-translate-y-1"
            >
              <div className="flex items-center justify-between mb-2.5">
                <sk.icon className={`${sk.color} text-2xl group-hover:scale-110 transition-transform`} />
                <span className="text-[11px] font-bold text-emerald-400 font-mono">{sk.level}</span>
              </div>
              
              <h3 className="text-xs sm:text-sm font-bold text-white font-['Sora'] mb-1.5 group-hover:text-emerald-300 transition-colors">
                {sk.name}
              </h3>
